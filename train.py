...
# Utility function to load a checkpoint dictionary from either a path or a dictionary.
...
def train(config, device, auto_remove_exp=False, resume=None):
    ...
    # 新增：保存最终 config
    with open(os.path.join(log_dir, 'final_config.json'), 'w') as f:
    ...
    try:
        ...
        print("Sample inspection failed:", e)
    ...
    for epoch in range(1, config.train.num_epochs + 1): # epoch numbers start at 1
        ...
        if torch.cuda.is_available():
            print("CUDA Memory Allocated: {:.2f} MB".format(torch.cuda.memory_allocated() / 1024 / 1024))
    # terminate logging
    data_logger.close()
...
def main(args):
    ...
    if args.config is not None:
        ...
        print("DEBUG ext_cfg:", ext_cfg)
        config = Config(ext_cfg) 
    ...
    if args.debug:
        ...
        # set batch size to 8 for debugging
        config.train.batch_size = min(config.train.batch_size, 8)
    ...
    try:
        ...
        res_str = "run failed with error:\n{}\n\n{}".format(e, traceback.format_exc())
                # 新增：写入日志
        log_dir = getattr(config, "train", {}).get("output_dir", "./")
