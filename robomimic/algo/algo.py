...
class Algo(object):
    ...
    def train_on_batch(self, batch, epoch, validate=False):
        ...
        """
        # if "actions" in batch:
        #     print("actions:", batch["actions"].min(), batch["actions"].max(), batch["actions"].mean())
        assert validate or self.nets.training
        # 检查 batch 是否有 NaN 或 Inf
        for k, v in batch.items():
            if torch.is_tensor(v):
                if torch.any(torch.isnan(v)) or torch.any(torch.isinf(v)):
                    print(f"WARNING: {k} contains NaN or Inf IN train_on_batch, min={v.min().item()}, max={v.max().item()}, mean={v.mean().item()}")
        return OrderedDict()
