...
def run_epoch(model, data_loader, epoch, validate=False, num_steps=None, obs_normalization_stats=None):
    ...
    for _ in LogUtils.custom_tqdm(range(num_steps)):
        ...
        input_batch = model.postprocess_batch_for_training(input_batch, obs_normalization_stats=obs_normalization_stats)
         # 检查 input_batch 是否有 NaN 或 Inf
        for k, v in input_batch.items():
            if torch.is_tensor(v):
                if torch.any(torch.isnan(v)) or torch.any(torch.isinf(v)):
                    print(f"WARNING: {k} contains NaN or Inf BEFORE train_on_batch, min={v.min().item()}, max={v.max().item()}, mean={v.mean().item()}")
        ...
        for k, v in batch.items():
            if torch.is_tensor(v) and (torch.any(torch.isnan(v)) or torch.any(torch.isinf(v))):
                print(f"WARNING: {k} contains NaN or Inf IN train_on_batch")
        # tensorboard logging
        t = time.time()
