...
def initialize_obs_utils_with_obs_specs(obs_modality_specs):
    ...
    Example:
        {
            "obs": {
                "low_dim": ["robot_eef_pos", "robot_eef_quat"],
                "rgb": ["agentview_image", "robot0_eye_in_hand"],
            }
            "goal": {
                "low_dim": ["robot_eef_pos"],
                "rgb": ["agentview_image"]
            }
        }
...
def get_processed_shape(obs_modality, input_shape):
    ...
    print("DEBUG normalization_stats:", normalization_stats)
    
def normalize_dict(dict, normalization_stats):
    for k in normalization_stats:
        if "offset" not in normalization_stats[k] and "mean" in normalization_stats[k]:
            normalization_stats[k]["offset"] = normalization_stats[k]["mean"]
        if "scale" not in normalization_stats[k] and "std" in normalization_stats[k]:
            normalization_stats[k]["scale"] = normalization_stats[k]["std"]
    """
    ...
    for k, v in dict.items():
        if isinstance(v, np.ndarray):
            if np.any(np.isnan(v)) or np.any(np.isinf(v)):
                # print(f"WARNING: input {k} contains NaN or Inf")
                pass
        if isinstance(v, np.ndarray):
            if np.any(np.isnan(v)) or np.any(np.isinf(v)):
                # print(f"WARNING: input {k} contains NaN or Inf")
    ...
    for m in dict:
        ...
        if isinstance(dict[m], np.ndarray):
            offset = np.asarray(offset)
            scale = np.asarray(scale)
            assert not np.any(np.abs(scale) < 1e-8), f"{m} scale异常: {scale}"
        else:
            offset = torch.as_tensor(offset, device=dict[m].device, dtype=dict[m].dtype)
            scale = torch.as_tensor(scale, device=dict[m].device, dtype=dict[m].dtype)
            assert not torch.any(torch.abs(scale) < 1e-8), f"{m} scale异常: {scale}"
        
        # 只检查最后一维
        assert dict[m].shape[-1] == offset.shape[-1], "feature dim mismatch in @normalize_dict"
        # 广播 offset/scale 到 dict[m] 的 shape
        offset = offset.reshape([1] * (dict[m].ndim - offset.ndim) + list(offset.shape))
        scale = scale.reshape([1] * (dict[m].ndim - scale.ndim) + list(scale.shape))
        dict[m] = (dict[m] - offset) / scale
        # shape consistency checks（移除严格 shape 检查）
        # m_num_dims = len(offset.shape)
        # shape_len_diff = len(dict[m].shape) - m_num_dims
        # print(f"DEBUG: dict[m].shape[-m_num_dims:]={dict[m].shape[-m_num_dims:]}, offset.shape={offset.shape}")
        # assert shape_len_diff >= 0, "shape length mismatch in @normalize_dict"
        # assert tuple(dict[m].shape[-m_num_dims:]) == tuple(offset.shape), "shape mismatch in @normalize_dict"
        ...
        # dict[m] = (dict[m] - offset) / scale
        # print(f"DEBUG: key={m}, dict[m].shape={dict[m].shape}, dict[m].dtype={dict[m].dtype}, offset.shape={offset.shape}, offset.dtype={offset.dtype}, scale.shape={scale.shape}, scale.dtype={scale.dtype}")
        # print(f"DEBUG: dict[m][-m_num_dims:]: {dict[m].shape[-m_num_dims:]}, offset.shape: {offset.shape}")
    return dict
...
def unnormalize_dict(dict, normalization_stats):
    ...
    for m in dict:
        ...
        if isinstance(dict[m], np.ndarray):
            ...
            scale = torch.as_tensor(scale, device=dict[m].device, dtype=dict[m].dtype)
        # 只检查最后一维
        assert dict[m].shape[-1] == offset.shape[-1], "feature dim mismatch in @unnormalize_dict"
        # 广播 offset/scale 到 dict[m] 的 shape
        offset = offset.reshape([1] * (dict[m].ndim - offset.ndim) + list(offset.shape))
        scale = scale.reshape([1] * (dict[m].ndim - scale.ndim) + list(scale.shape))
