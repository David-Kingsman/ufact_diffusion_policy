...
class SequenceDataset(torch.utils.data.Dataset):
    ...
    @staticmethod
    def _random_pose(x_min=-0.05, x_max=0.05, y_min=-0.04, y_max=0.04, z_min=-0.04, z_max=0.05, z_theta_min=-np.pi/3, z_theta_max=np.pi/3):
    ...
    def _augmentation_pose(self, meta):
        ...
        if 'robot_eef_pos' in meta['obs'].keys():
            meta['obs']['robot_eef_pos'][:, :3] = meta['obs']['robot_eef_pos'][:, :3] @ pose[:3, :3].T + pose[:3, 3].reshape(1, 3)
        if 'robot_eef_quat' in meta['obs'].keys():
            meta['obs']['robot_eef_quat'][:, :4] = Rot.from_matrix(pose[:3, :3] @ Rot.from_quat(meta['obs']['robot_eef_quat'][:, :4]).as_matrix()).as_quat()
        if 'pointcloud' in meta['obs'].keys():
            meta['obs']['pointcloud'][:, :, :3] = meta['obs']['pointcloud'][:, :, :3] @ pose[:3, :3].T + pose[:3, 3].reshape(1, 3)

        if 'actions' in meta.keys():
            meta['actions'][:, :3] = meta['actions'][:, :3] @ pose[:3, :3].T + pose[:3, 3].reshape(1, 3)
            meta['actions'][:, 3:7] = Rot.from_matrix(pose[:3, :3] @ Rot.from_quat(meta['actions'][:, 3:7]).as_matrix()).as_quat()
        return meta
...
def action_stats_to_normalization_stats(action_stats, action_config):
    ...
    for action_key in action_stats.keys():
        ...
        if norm_method is None:
            ...
            # TODO: hardcode for pose augmentation
            input_min[0, :3] -= np.array([0.2, 0.4, 0.2])
        ...
        print(f"DEBUG: {action_key} normalization_stats: {action_normalization_stats[action_key].keys()}")
