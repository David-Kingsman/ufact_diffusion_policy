...
@click.option('--robot_ip', '-ri', default ='192.168.1.217',required=True, help="UR5's IP address e.g. 192.168.0.204")
...
@click.option('--frequency', '-f', default=25, type=float, help="Control frequency in Hz.")
...
def main(input, output, robot_ip, match_dataset, match_episode,
    ...
    with SharedMemoryManager() as shm_manager:
        with KeystrokeCounter() as key_counter, \
             Spacemouse(shm_manager=shm_manager) as sm, RealEnv(
            output_dir=output, 
            robot_ip=robot_ip, 
            frequency=frequency,
            ...
            while True:
                ...
                target_pose = state['TargetTCPPose'] #pose is (x,y,z,rx,ry,rz)
                ...
                while True:
                    ...
                    # print(f'policy_control:{policy_control}')

                    precise_wait(t_sample)
                    # get teleop command
                    sm_state = sm.get_motion_state_transformed()
                    # print(sm_state)
                    # print(env.max_pos_speed)
                    ...
                    # clip target pose
                    # print(f'target_pose:{target_pose[:2]}')
                    # for safety?????? ur unit is in meter and xarm unit is in mm!!!!!
                    target_pose[:2] = np.clip(target_pose[:2], [170, -225], [500, 200])
                ...
                try:
                    # start episode
                    print(f'enter policy control loop')
                    policy.reset()
                    start_delay = 1.0
                    ...
                    while True:
                        ...
                        this_target_poses[:,:2] = np.clip(
                            this_target_poses[:,:2], [170, -225], [500, 200])

                        # execute actions
                        this_target_poses[:,2] = 0
